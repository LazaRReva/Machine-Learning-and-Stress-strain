...
def plot_prediction_vs_actual(model, original_grains_data, scaler_x, scaler_y):
    # (此函数已更新，以使用动态应变范围)
    print("开始为真实晶粒绘制'预测vs实际'对比图(已优化)...")
    for i, grain_data in enumerate(original_grains_data):
        plt.figure(figsize=(10, 8))

        # 1. 绘制真实的、清理后的实验数据散点
        plt.scatter(grain_data['exx'], grain_data['sxx'], s=15, alpha=0.4, color='blue',
                    label='Cleaned Experimental Data')
        ...
        # 4. 绘制模型的预测曲线
        plt.plot(strain_range, stress_predicted, color='red', linewidth=3,
                 label=f'Model Prediction for Avg. Orientation')

        plt.title(f'Prediction vs. Actual for Grain {i + 1} (Cleaned Data)', fontsize=16)
        plt.xlabel('Strain (Exx)', fontsize=12);
        plt.ylabel('Stress (Sxx) [MPa]', fontsize=12)
        plt.legend();
        plt.grid(True, linestyle='--', alpha=0.6)
        plt.savefig(f"prediction_vs_actual_grain_{i + 1}_cleaned.png", dpi=300)
        plt.show()
...
if __name__ == '__main__':
    ...
    # 1. 加载并清理数据
    X_tensor, Y_tensor, original_grains = load_and_prepare_fcnn_data(MAT_FILE, perform_cleaning=True)
