...
class SimpleDataset(Dataset):
    def __init__(self, sigma, epsilon):
        self.sigma = torch.tensor(sigma, dtype=torch.float32).unsqueeze(1)
        self.epsilon = torch.tensor(epsilon, dtype=torch.float32).unsqueeze(1)
...
dataset = SimpleDataset(sigma, epsilon)
...
sigma_test_tensor = torch.tensor(sigma_test, dtype=torch.float32).unsqueeze(1)
epsilon_test_tensor = torch.tensor(epsilon_test, dtype=torch.float32).unsqueeze(1)
...
plt.subplot(1, 2, 1)
plt.plot(epsilon_test, label='Actual epsilon')
plt.legend()
plt.title('Actual epsilon')

plt.subplot(1, 2, 2)
plt.plot(predictions, label='Predicted epsilon')
plt.legend()
