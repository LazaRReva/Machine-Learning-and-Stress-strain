...
sigma = np.random.rand(10000, 3, 3)  # 创建应力数据的示例数组
epsilon = np.random.rand(10000, 3, 3)  # 创建应变数据的示例数组
...
with torch.no_grad():  # 在此上下文管理器内，停止自动求导机制
    test_sigma = np.random.rand(10, 3, 3)  # 创建一个形状为 (10, 3, 3) 的 numpy 数组，包含10个样本的测试应力数据，每个样本是一个 3x3 矩阵
    test_sigma_tensor = torch.tensor(test_sigma, dtype=torch.float32)  # 将 numpy 数组 test_sigma 转换为 PyTorch 张量，并将数据类型设为 float32
    predictions = model(test_sigma_tensor)  # 通过模型前向传播计算预测结果
